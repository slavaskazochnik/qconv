...
IS
  'Логин пользователя' ;
  COMMENT ON COLUMN USER_ACCOUNT.PASSW
IS
  'Пароль' ;
  COMMENT ON COLUMN USER_ACCOUNT.EMAIL
IS
  'Контактный email' ;
  COMMENT ON COLUMN USER_ACCOUNT.APP_ROLE_ID
IS
  'Роль пользователя' ;
  COMMENT ON COLUMN USER_ACCOUNT.PERSON_ID
IS
  'ID персоны' ;
  COMMENT ON COLUMN USER_ACCOUNT.SIGN_ACTIVE
IS
  'Признак активности' ;
  COMMENT ON COLUMN USER_ACCOUNT.D_CREATE
IS
  'Дата создания' ;
  ALTER TABLE USER_ACCOUNT ADD CONSTRAINT USERS_ACCOUNT_PK PRIMARY KEY ( ID ) ;
  ALTER TABLE USER_ACCOUNT ADD CONSTRAINT USER_ACCOUNT_UK UNIQUE ( NAME ) ;
...
ALTER TABLE USER_ACCOUNT ADD CONSTRAINT USER_ACCOUNT_APP_ROLE_FK FOREIGN KEY ( APP_ROLE_ID ) REFERENCES APP_ROLE ( ID ) ;

ALTER TABLE USER_ACCOUNT ADD CONSTRAINT USER_ACCOUNT_USER_PROFILE_FK FOREIGN KEY ( PERSON_ID ) REFERENCES USER_PROFILE ( ID ) ;
